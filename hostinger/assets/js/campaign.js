/* campaign.js — start/pause campaign + validate all + status indicator */
(function () {
  ...
  async function start() {
    UI.modal({
      ...
      body: `
        <p class="text-sm text-ink-700">This will queue all <b>valid</b> leads for first outreach and send first batch immediately.</p>
        <ul class="text-xs text-ink-500 mt-3 space-y-1 list-disc pl-5">
          ...
          <li>First 5 leads sent immediately</li>
      ...
      onConfirm: async () => {
        try {
          ...
          UI.toast(`Campaign started! ${r.queued} queued, ${r.sent_now || 0} sent now`);
  ...
  window.CAMPAIGN = { init, start, pause, validateAll, updateStatusBanner };
})();
